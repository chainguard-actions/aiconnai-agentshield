...
reports. The current release line is `0.9.3`.
...
```text
ghcr.io/aiconnai/agentshield:0.9.3
```
...
```bash
docker pull ghcr.io/aiconnai/agentshield:0.9.3
docker run --rm -v "$PWD:/scan" ghcr.io/aiconnai/agentshield:0.9.3 scan .
docker run --rm ghcr.io/aiconnai/agentshield:0.9.3 --version
```
